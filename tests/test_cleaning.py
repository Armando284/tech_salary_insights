...
def test_clean_column_names(sample_dataframe):
    ...
    expected_columns = ['column_a', 'column_b', 'column_c', 'annual_base_pay', 'signing_bonus', 'invalid_col']
...
def test_remove_invalid_rows(sample_dataframe):
    ...
    assert df['invalid_col'].str.contains(r'[^\x20-\x7E]', regex=True, na=False).sum() == 0
